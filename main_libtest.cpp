...
void DoSATDTestIntra(unsigned char* address1,int stride1, unsigned char* address2,int stride2,unsigned int* orio,unsigned int* newo)
{
	UINT so,eo,sn,en;
				...
				for( int mvy=0;mvy<16;mvy+=4)
					for( int mvx=0;mvx<16;mvx+=4)
							...
							&& mbx * MB_SIZE + mvx >= 0 && (MB_X_REF-1) * MB_SIZE > mvx + mbx * MB_SIZE )
						{
							...
							for( int loop=0;loop<LOOP_TEST_COUNT_INTRA;loop++)
							{
								unsigned int tres = SATD_16x16_INTRA_C( refa, stride1, cura, stride2 );
...
//								unsigned int tres = SATD4x4_ASM_MMX( refa, stride1, cura, stride2 );
								unsigned int tres = SATD_16x16_INTRA_intrinsic( refa, stride1, cura, stride2 );
								if( mvy >=0 && mvy < MB_SIZE && mvx >=0 && mvx < MB_SIZE )
								{
/*
...
unsigned int SATD_4x4_CACHED1( unsigned char *address1, unsigned char *address2 )
{
...
unsigned int SATD_4x4_Intrinsic1( unsigned char *address1, unsigned int stride1, unsigned char *address2, unsigned int stride2 )
{
	const unsigned char *pSrc1;
	int src1Step;
	const unsigned char *pSrc2;
...
unsigned int SATD_8x8( unsigned char *address1, unsigned int stride1, unsigned char *address2, unsigned int stride2 ){
...
/*	if( CPU_HAS_SSE4_1() )
	{
		functions[0] = SATD_4x4_Intrinsic;
		functions[1] = satd_4x4_sse4_YASM;
	}
	else if( CPU_HAS_SSE3() )
	{
		functions[0] = SATD_4x4_Intrinsic;
		functions[1] = satd_4x4_sse3_YASM;
	}
	else
	{
		printf("OMG how did we end up here ? \n");
		functions[0] = SATD_4x4_Intrinsic;
		functions[1] = SATD4x4_INTEL_Intrinsic;
	}
	/**/
/*	if( CPU_HAS_SSE4_1() )
	{
		functions[0] = SATD_8x8;
		functions[1] = satd_8x8_sse4_YASM;
	}
	else if( CPU_HAS_SSE3() )
	{
		functions[0] = SATD_8x8;
		functions[1] = satd_8x8_sse3_YASM;
	}
	else
	{
		printf("OMG how did we end up here ? \n");
		functions[0] = SATD_8x8;
		functions[1] = SATD_8x8;
	}/**/
	...
	{
		functions[0] = SATD_16x16_SSE2_Intrinsic;
		functions[1] = satd_16x16_sse3_YASM;
	}
	else
	{
		printf("OMG how did we end up here ? \n");
...
//	functions[0] = SAD_16x16_intrinsic;
//	functions[1] = sad_16x16_sse2_ASM;
//	DoSATDTestIntra( address1, stride, address2, stride, orio, newo );
